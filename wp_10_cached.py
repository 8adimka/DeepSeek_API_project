...
Краткое резюме:"""
        ...
        # Здесь должен быть вызов LLM для суммаризации
        # Временная заглушка - простая текстовая суммаризация
        summary = self._simple_text_summary(qa_entries)
    ...
    def _simple_text_summary(self, qa_entries: List[QAEntry]) -> str:
        """Простая текстовая суммаризация (заглушка)"""
        topics = set()
        for entry in qa_entries:
            # Извлекаем ключевые слова из вопросов и ответов
        ...
        return f"Обсуждаемые темы: {', '.join(sorted(list(topics))[:10])}"
...
Подготовь краткий тезисный ответ на вопрос в контексте программирования на Python. Учти контекст предыдущего обсуждения. Ответь по порядку, только ключевые пункты, без введения, заключения и лишних слов, постарайся при формировании ответа использовать наиболее подходящие технологии, фреймворки и практики. Если потребуется писать код, то пиши его на Python."""
        else:
            prompt = (
                f"Подготовь краткий тезисный ответ на вопрос в контексте программирования на Python: {question}\n\n"
                "Ответь по порядку, только ключевые пункты, без введения, заключения и лишних слов, постарайся при формировании ответа использовать наиболее подходящие технологии, фреймворки и практики. Если потребуется писать код, то пиши его на Python."
            )
