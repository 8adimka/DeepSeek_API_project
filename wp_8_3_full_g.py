...
def _extract_text_from_obj(obj: Any) -> Optional[str]:
    ...
    if isinstance(obj, dict):
        ...
        return " ".join(
            [
                s
                for s in (x.strip() for x in out if isinstance(x, str) and x.strip())
                if s
            ]
        )
...
class DeepSeekSolver:
    ...
    def send_to_api_interview(
        ...
        try:
            response = self._session.post(self.API_URL, json=data, timeout=timeout)
            response.raise_for_status()
            self.last_request_time = time.time()
    ...
    def process_interview_question(self, question: str) -> None:
        ...
        answer = self.send_to_api_interview(prompt)
