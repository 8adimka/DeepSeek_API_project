...
class DialogueContextManager:
    """Менеджер контекста диалога в памяти"""

    def __init__(
        self,
        max_recent_entries: int = 8,
        max_tokens: int = 800,
        summarization_threshold: int = 700,
        ...
        self.min_summarization_interval: float = 60  # 1 minute
    ...
    def _start_summarization_worker(self):
        """Запуск фонового потока для суммаризации"""

        def worker():
            while True:
                try:
                    qa_entries = self._summarization_queue.get(
                        timeout=300
                    )  # 5min timeout
                    if qa_entries is None:  # Sentinel for shutdown
                        break
    ...
    def _estimate_tokens(self, text: str) -> int:
        """Оценка количества токенов в тексте (упрощённая)"""
    ...
    def _check_summarization_needed(self) -> None:
        ...
        if (
            ...
            # Копируем данные для суммаризации
    ...
    def _perform_summarization(self, qa_entries: List[QAEntry]) -> None:
        """Выполнение суммаризации через LLM"""
        if not qa_entries:
            return

        # Создаем промпт для суммаризации
        dialogue_text = self._format_qa_for_summarization(qa_entries)

        summarization_prompt = f"""Суммаризируй следующий диалог в контексте программирования на Python, сохраняя ключевые технические детали, решения и важные моменты. Сделай краткое, информативное резюме на русском языке.
...
Краткое резюме:"""
        ...
        if self.solver:
            summary = self.solver.send_to_api(summarization_prompt, max_tokens=300)
        else:
            summary = ""  # Если solver не предоставлен, пропускаем суммаризацию
        ...
        if summary:
            with self._summarization_lock:
                ...
                # Очищаем старые записи, оставляя только самые свежие
                keep_count = min(2, len(self.recent_qa))
                if len(self.recent_qa) > keep_count:
                    removed_entries = self.recent_qa[:-keep_count]
                    self.recent_qa = self.recent_qa[-keep_count:]

                    # Пересчитываем токены
                    self._token_count = sum(entry.tokens for entry in self.recent_qa)
                    self._token_count += self._estimate_tokens(self.summary)

    def _format_qa_for_summarization(self, qa_entries: List[QAEntry]) -> str:
        """Форматирование QA для суммаризации"""
        lines = []
        for i, entry in enumerate(qa_entries, 1):
            lines.append(f"Вопрос {i}: {entry.question}")
            lines.append(f"Ответ {i}: {entry.answer}")
            lines.append("")
        return "\n".join(lines)

    def get_context_for_query(self, new_question: str) -> str:
        """Получение релевантного контекста для нового вопроса"""
        context_parts = []

        # Добавляем суммаризацию, если есть
        if self.summary:
            context_parts.append(f"Контекст предыдущего обсуждения:\n{self.summary}\n")

        # Добавляем последние QA пары
        if self.recent_qa:
            context_parts.append("Последние вопросы и ответы:")
            for entry in self.recent_qa[-3:]:  # Последние 3 пары
                context_parts.append(f"В: {entry.question}")
                context_parts.append(f"О: {entry.answer}")
...
class DeepSeekSolver:
    ...
    def process_interview_question(self, question: str) -> None:
        ...
        # Получаем контекст для нового вопроса
        context = ""
        if self.context_manager:
            context = self.context_manager.get_context_for_query(question)

        # Формируем промпт с контекстом
        if context:
            prompt = f"""Контекст предыдущего обсуждения:
{context}

Новый вопрос: {question}

Подготовь краткий тезисный ответ на вопрос в контексте программирования на Python. Учти контекст предыдущего обсуждения. Ответь по порядку, только ключевые пункты, без введения, заключения и лишних слов. Если потребуется писать код, то пиши его на Python."""
        else:
            prompt = (
        ...
        # Добавляем в контекст, если есть менеджер контекста
...
Подготовь краткий тезисный ответ на вопрос как будто ты опытный программист со стажем более 10 лет в сфере IT. Учти контекст предыдущего обсуждения. Ответь по порядку, только ключевые пункты, без введения, заключения и лишних слов, постарайся при формировании ответа использовать наиболее подходящие технологии, фреймворки и практики. Пиши код, только если это нужно для ответа на вопрос, но если потребуется писать код, то пиши его на Python."""
        else:
            prompt = (
                f"Подготовь краткий тезисный ответ на вопрос как будто ты опытный программист со стажем более 10 лет в сфере IT: {question}\n\n"
                "Ответь по порядку, только ключевые пункты, без введения, заключения и лишних слов, постарайся при формировании ответа использовать наиболее подходящие технологии, фреймворки и практики. Пиши код, только если это нужно для ответа на вопрос, но если потребуется писать код, то пиши его на Python."
            )
...
def run_daemon():
    ...
    # Используем OpenAISolver для аудиовопросов (NumLock) с передачей менеджера контекста
    audio_solver = OpenAISolver(
        telegram_sender_instance=telegram_sender, context_manager=context_manager
    )

    # Передаём solver в context_manager для суммаризации
    context_manager.solver = audio_solver
    ...
    def process_audio_question():
        ...
        if question:
            # Используем OpenAISolver для обработки аудиовопросов с контекстом
